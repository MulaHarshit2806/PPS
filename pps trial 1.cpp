...
int main()
{
     ...
     {
      printf("\t***********************************\n");
      printf("\t'WELCOME TO SRM HOSTEL DATABASE'\n");
      printf("\t***********************************\n\n\n");
...
void insert()//declaration
{
    FILE *fp; 
     ...
     //storage perform in roomondary memory,file creation happens in 2nd memomry but all func happens in main memory
    ...
    printf("\n\t\tEnter Student hostel_name : ");
    gets(s.hostel_name);
    printf("\n\t\tEnter Student room : ");
    gets(s.room);
    printf("\n\t\tEnter Student Total Fees Paid : ");
    scanf("%d",&s.fees_paid);
    ...
    while(fread(&s,sizeof(s),1,fp)>0&&flag==0)
    {
...

        {
            flag=1;
             ...
             printf("\nRoll No\t\thostel_name of Student\t\troom\t\tfees_paid\n");
             printf("%d\t\t\t%s  \t\t  %s \t\t%d\n\n\n",s.roll,s.hostel_name,s.room,s.fees_paid);
        }
    }
        if(flag==0)
        {
...
void update()
{
    ...
    while(fread(&s,sizeof(s),1,fp)>0 && flag==0)
    {
        if(s.roll==ro)
        {
            ...
            scanf("%d",&s.roll);
            fflush(stdin);
            printf("\n\t\tUpdated Student hostel_name: ");
            gets(s.hostel_name);
            printf("\n\t\tUpdated Student room: ");
            gets(s.room);
            printf("\n\t\tUpdated Student fees_paid: ");
            scanf("%d",&s.fees_paid);
